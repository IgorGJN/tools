const TaskUI = (function () {
  ...
  function renderHashtags(tags, activeTag) {
    if (!tags.length) {
      elements.hashtagsPanel.innerHTML = '<span class="tag-empty">Nenhuma hashtag com pendências</span>';
      return;
    }

    let html = "";

    html += '<button class="tag-chip' + (activeTag === "" ? " active" : "") + '" data-tag="">Todas</button>';
  ...
  function renderTasks(tasks, currentUsername) {
    elements.taskCountLabel.textContent = tasks.length + " " + (tasks.length === 1 ? "item" : "itens");
    ...
    tasks.forEach(function (task) {
      ...
      const isMine = String(task.owner || "") === String(currentUsername || "").toLowerCase();
      ...
      html += '<span class="status-badge ' + meta.className + '">' + meta.label + "</span>";
      html += '<span class="meta-pill">' + escapeHtml(formatDateRange(task)) + "</span>";
      html += '<span class="owner-pill' + (isMine ? '' : ' shared') + '">' + (isMine ? 'Minha' : 'Compartilhada por @' + escapeHtml(task.owner || '')) + '</span>';

      if (recurrenceText) {
        html += '<span class="meta-pill recurrence-pill">' + escapeHtml(recurrenceText) + "</span>";
      }
  ...
  function renderUpcoming(tasks) {
    if (!tasks.length) {
      elements.upcomingList.innerHTML = '<div class="upcoming-card"><p class="task-description">Nenhuma tarefa pendente.</p></div>';
      return;
    }
    ...
    tasks.forEach(function (task) {
      ...
      html += '<div class="upcoming-date">' + escapeHtml(formatDateRange(task)) + "</div>";
      html += "</article>";
    });
